    ...
    return (
        <div className='rounded-md overflow-hidden'>
            ...
            <div className='relative group rounded-md overflow-hidden cursor-pointer'>
                ...
                <Image height={250} width={230} className='max-h-[250px] object-cover group-hover:scale-[1.1] transition-all z-[1]' src={item?.image} alt='image' />
                <div className='absolute top-0 right-0 z-[4] '>
                    <Chip color='warning' radius='none' className='px-1 rounded-bl-xl'>{item?.subOrDub}</Chip>
                </div>
                {!isPopular && <div className='text-[13px] bg-black/80 flex justify-center items-center px-2 py-2 absolute bottom-0 left-0 w-full'>
                    {item?.name}
                </div>}
            </div>
